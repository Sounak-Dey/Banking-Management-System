...
char* strfromlonglong(long long int value)		
{
  ...
  p = buf + 31;
  do						// storing digits in p.....digit by digit
  {
    *p -- = '0' + (v%10);
    v /= 10;
  } while(v);
...
// get current balance value from DB
...
char usrupdateu[] = "Update UnsucessFull Low balance\n--------------------\n\nWrite exit for quitting.";
	...
	while(1){
		...
		if(balance >= amount){
			...
			sendMsgtoClient(client_fd,usrupdate);    // sending update successfull msg
		}
			
		else{
			sendMsgtoClient(client_fd,usrupdateu);	// send update unsuccessfull msg	
		}
...
void AddUser(int client_fd)
{
    ...
    { 
		...
		int fd = open("login.dat", O_WRONLY | O_APPEND);        // create entry in login.dat
...
void jointAcc(int client_fd)
{
	...
	if(strcmp(pass1,cpass) == 0) // 1st user's credentials verified
    {
		name2 = NULL;
		cpass = NULL;
		...
		if(strcmp(pass2,cpass)==0)  // user 2 password verified	
        {
			...
			// account name will be user1's name. when user2 will login, it'll access the respective user1's file.
		...
		else        // 2nd user's password's don't match
        {
			sendMsgtoClient(client_fd, "Passwords do not match\nEnter New Password::");
			goto PASS2;			
		}
	}	
	else        // 1st user's passwords don't match
    {
		sendMsgtoClient(client_fd, "Passwords do not match\nEnter New Password::");
		goto PASS;			
	}
	...
	while(read(fd, &usr, sizeof(usr)) > 0)
    {
		if(strcmp(usr.username, username) == 0)
        {
			...
			if(strcmp(pass,cpass) == 0)
            {
				...
				strcpy(usr.password, pass);
				write(fd, &usr, sizeof(usr));
				sendMsgtoClient(client_fd, usrupdate);		// sending user msg about successfull update
				return 1;		
			}
			else
            {
	...
	while(flag)
	{
		...
		switch(choice)
		{
			...
			case 7:
				...
				break;
			default:
				sendMsgtoClient(client_fd, "Unknown Query, Please Choose from the given Options");
				break;
		}
	}
}

// To manage Admin Requests
		...
		if(strcmp(usr.username,username)==0){
			if(strcmp(usr.password,password)==0){		
                if(usr.type=='C' || usr.type == 'J'){	
					// checking for active status of account
					if(usr.active=='y'){
						printf("\nAccepting\n");
						close(fd);
                    	return USER;
...
void talkToClient(int client_fd)
{
	...
	utype=authorize(username,password); // validate and authorize
	...
	switch(utype)
	{
		...
		case UNAUTH_USER:
			closeclient(client_fd,"unauthorised access, Wrong Credentials");
			break;
		default:
			closeclient(client_fd,"unauthorised access, Wrong Credentials");
			break;
...
int main(int argc,char **argv)
{
	...
	serv_addr.sin_family = AF_INET;             //setting DOMAIN as IPV4
	serv_addr.sin_addr.s_addr = INADDR_ANY;     //permits any incoming IP

	if(bind(sock_fd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
	    printf("Error on binding....Port Number may not be available.\n");
	    exit(EXIT_FAILURE);
	}
	int reuse=1;
